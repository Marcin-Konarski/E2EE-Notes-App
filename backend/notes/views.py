...
from .serializers import NotesSerializer, NotesDetailSerializer, SetEncryptionSerializer, ShareSerializer
...
class NotesViewSet(CreateModelMixin, RetrieveModelMixin, UpdateModelMixin, DestroyModelMixin, GenericViewSet): # This endpoint also supports the POST request
    ...
    def set_encryption(self, request, pk=None):
        ...
        if enabled: # If encryption key is provided than create NoteItem object for this note
            ...
            try:
                ...
                NoteItem.objects.filter(note=note).delete() # If Note is not to be encypted than remove encryption for all users who have access to this note
            except NoteItem.DoesNotExist:
                pass # If no queried records in NoteItem exist than there is no need to do anything since we just want to delete them
    ...
    def share(self, request, pk=None):
        ...
        if note.owner.id != request.user.id and current_user_permission not in ['S', 'O']: # Ensure only owner can share notes or user has sufficient permissions. 'O' means Owner permissions
            return Response({'detail': 'Only owner and users with owner \'permissions\' can share notes'}, status=status.HTTP_403_FORBIDDEN)
